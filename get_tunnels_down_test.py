                ...
                if appliance_id == "145.NE":
                    try:
                        ...
                        for tunnel_id, tunnel_info in tunnels.items():
                            ...
                            message = {
                            "text": f"WARNING_test: On {appliance_name}, the tunnel {dest_tunnel} is {status}."
                            }
                            #with open("DOWNtunnels.json", "r", encoding="utf-8") as f:
                                #DOWNtunnels = json.load(f)  # 返回字典或列表
                            with open("DOWNtunnels.txt", 'r') as f:
                                content = f.read()    
                                if dest_tunnel in content:
                                    print(f"{dest_tunnel}.")
                                else:
                                    print(f"警告：键 '{dest_tunnel}' 不存在")   
                                    try:
                        ...
                        #with open("DOWNtunnels.json", "w") as f:
                            #json.dump(tunnels, f, ensure_ascii=False, indent=2)
                        with open("DOWNtunnels.txt", "w", encoding="utf-8") as f:
                            json_str = json.dumps(tunnels, ensure_ascii=False, indent=2)
                            f.write(json_str)
                    except Exception as e:
                        print(f"Error: {e}")
